...
// Variabel default untuk mencegah error
$articles = [];

// Coba koneksi database hanya jika file config.php ada
if (file_exists('config.php')) {
    require_once 'config.php';
    
    // Pastikan koneksi PDO dibuat sebelum digunakan
    if (isset($pdo)) {
        try {
            $stmt = $pdo->query("SELECT * FROM articles ORDER BY created_at DESC");
            $articles = $stmt->fetchAll();
        } catch (PDOException $e) {
            // Jika query gagal, tetap gunakan array kosong
            $articles = [];
        }
    }
}
